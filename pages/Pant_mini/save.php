          ...
          <div class="col-sm-6">
            ...
            if($r==null){
              ...
              while($val=mysqli_fetch_assoc($yy)){
                    ...
                    $in="SELECT quantity from paintmini_out WHERE paint_main_id='$item_id' and date='$date' and reqdepartment='3'";//out from paint mini store
                   $in1=mysqli_query($conn,$in) or die(mysqli_error($conn));
                   if(mysqli_num_rows($in1)>0){
                    while( $i1=mysqli_fetch_assoc($in1)){$o=$o+$i1['quantity'];}
                   }
                   else{$o=0;}
                   $out="SELECT quantity from main_out WHERE paint_main_id='$item_id' and date='$date' and reqdepartment='2'";//for paint mini in
                   $out1=mysqli_query($conn,$out) or die(mysqli_error($conn));
                   if(mysqli_num_rows($out1)>0){
                    while($ot1=mysqli_fetch_assoc($out1)){
                      $i=$i+$ot1['quantity'];
                    }
